...
import { useInvoicingPlan, useInvoicingPlanMutations } from '@/features/invoicing-plans/api';
import { useRouter } from '@/lib/i18n/navigation';

export const InvoicingPlanDetail = ({ code }: { code: string }) => {
  const { data, isLoading, isError } = useInvoicingPlan(code);
  const { remove } = useInvoicingPlanMutations();
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, md: 6 }}>
          <Card>
            <CardContent>
              <Stack spacing={1.5}>
                <Row label={t('forms.invoicingPlan.description')} value={data.description} />
                <Row label={t('forms.invoicingPlan.updatedCode')} value={data.updatedCode} />
              </Stack>
            </CardContent>
          </Card>
        </Grid>
